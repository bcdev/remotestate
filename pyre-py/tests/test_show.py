...
from pyre.show import _get_cell_id, _in_jupyter, _wait_for_port_free
...
def test_in_jupyter_returns_true_when_ipython_active():
    mock_ip = MagicMock()
    with patch("pyre.show._get_ipython", return_value=mock_ip):
        assert _in_jupyter() is True


def test_in_jupyter_returns_false_when_no_ipython():
    with patch("pyre.show._get_ipython", return_value=None):
        assert _in_jupyter() is False


def test_in_jupyter_returns_false_on_exception():
    with patch("pyre.show._get_ipython", side_effect=RuntimeError("oops")):
        assert _in_jupyter() is False
